...
export default {
    ...
    methods: {
        ...
        changeCategory(category, categoryid, from){ //展示当前选中子节点的信息,from=elTab表示通过切换tab来选中子节点
            ...
            if(category != null)
                this.$emit('changeCategory', category, categoryid, from)
        },
        toggleShow(item) {
        ...
        deleteCategory() {
            const category = this.getCategory('operate')
            if(this.currentOperateCategoryId.length === 2){
                Service.deleteOneStrategy(this.categoryList, category, this, this.checkCategory)
            }else{
        ...
        rightMenu(event, id) {
            ...
            if(id.length === 1){
                ...
                this.nextLevelChildOprTitle = '新建指标'
            }else{
                this.sameLevelChildOprTitle = '新建指标'
        ...
        //查询所有的分类
        queryGroupList(){
            let url = urlObj.apiGroupList
            if(this.isStrategyComp){
                url = urlObj.apiAlgorithmGroupList
            }
            ...
            .then(res => {
                ...
                if(res.code === 200){
                    this.categoryList = res.data.map(item => {
                        ...
                        let children = []
                        if(this.isStrategyComp){
                            children = item.algorithms.map(item => {
                                let sub = defaultData.getListSubItem()
                                sub.categoryid = stra.id
                                sub.name = stra.name
                            })
                        }else{
                            children = item.strategies.map(stra => {
                                let sub = defaultData.getListSubItem()
                                sub.groupId = group.categoryid
                                sub.categoryid = stra.id
                                sub.name = stra.name
                                return sub
                            })
        ...
        queryStrategyOpr(category) {
            ...
            urlObj.post(`${urlObj.apiStrategyOpr}${category.categoryid}`, data, res => {
                if(res.code === 200){
                    ...
                    this.sending = false;
                    this.text = '';
                    const parent = find(this.categoryList, item => item.categoryid === category.groupId)
                    const index = findIndex(parent.children, item => item.categoryid === category.categoryid)
                    parent.children.splice(index, 1, category)
                    //页面更新这个策略的数据
                }else{
                    this.$message({message: res.data.message || '修改失败', type:'warning', duration: 2000});
                }
            })
        },
        sendForSelectStockStrategy(){ //针对于策略编辑组件
            ...
            if(category.editing){
                this.sending = true;
                if(this.currentOperateCategoryId.length === 2){
                    Service.updateOneStrategy(category, null, this, this.checkCategory)
                }else{
                ...
                if(this.createType == 'brother'){
                    ...
                    if(this.currentOperateCategoryId.length === 1){
                        ...
                        const len = parent.length;
                        newCategory = parent[len-1];
                        Service.addOneStrategy(newCategory, this, this.checkCategory)
                    }
                }else if(this.createType == 'child'){
                    // const len = category.children.length;
