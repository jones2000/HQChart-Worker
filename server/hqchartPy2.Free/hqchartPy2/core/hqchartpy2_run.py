...
from hqchartPy2.core.hqchartpy2_hqchartresult import HQChartResult, HQSelectResult
...
class HQChartPySimpleRun(IHQChartPyRun):
    ...
    def Run(self):
        ...
        # 设定日期范围
        self.m_HQData.SetDateRange(self.m_nStartDate, self.m_nEndData)
        # 设定计算条目
        self.m_HQData.m_nCalculateCount = self.m_nCalculateCount

        res = FastHQChart.Run(jsConfig, self.m_HQData, proSuccess=self.m_Result.RunSuccess,
                              procFailed=self.m_Result.RunFailed)
...
class HQChartPyMultiRun(IHQChartPyRun):
    ...
    def Run(self):
        ...
        self.m_nRunTick = (time.process_time() - self.m_StartTime)
        log = "[HQChartPyMultiRun::Run] Time used:{0}, symbol count:{1}".format(self.m_nRunTick, count)
        print(log)
...
class HQChartPyPolicyRun(IHQChartPyRun) :
    ...
    def SetPostData(self, request) :
        ...
        stockPoolList=requestData['stockPool'] # 股票池列表
        symbolList = self.get_symbol_list(stockPoolList)
        self.m_arySymbol=symbolList
    ...
    def Run(self):
        ...
        for i in range(len(self.AryIndex)) :
            ...
            jsConfig = self.GetRunConfig(item, item.get("ID",i))      # 运行配置项
            if (jsConfig==None):
                continue
